...
const eventApiKey = encyption.decrypt(config.application.urls.eventApiKey, config.secret);
...
function getHeaders(type, apikeyType, agentRefObject) {
  ...
  if (apikeyType === 'batch') {
    ...
    headers.apiKey = eventApiKey;
  } else if (apikeyType === 'filter') {
    headers.apiKey = config.application.urls.filterApiKey;
  } else {
