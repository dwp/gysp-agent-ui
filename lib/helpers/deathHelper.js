...
const deathMaxNoRepayAmount = 25;
...
module.exports = {
  ...
  successMessage(verification, status, section) {
    ...
    if (status === OVERPAYMENT) {
      if (section === 'retryCalc' || section === 'canVerifyDateOfDeath' || section === 'dapOnly') {
  ...
  isOriginDapOnly(origin) {
    return origin === 'dapOnly';
  },
  ...
  isDeathOverpaymentRepayable(amount) {
    if (typeof amount !== 'number') {
      throw new TypeError(`Expected number got ${typeof amount}: ${amount}`);
    }
    return Math.abs(amount) > deathMaxNoRepayAmount;
  },
  isDeathOverpaymentPeriods(periods) {
