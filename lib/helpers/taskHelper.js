...
const taskDeathArrearsDetailObject = require('../objects/view/taskDeathArrearsDetailObject');
const taskDeathOverPaymentDetailObject = require('../objects/view/taskDeathOverPaymentDetailObject');
const taskDeathOverPaymentEmailDetailObject = require('../objects/view/taskDeathOverPaymentEmailDetailObject');
const taskSrbOverPaymentDetailObject = require('../objects/view/taskSrbOverPaymentDetailObject');
...
const isDeathArrearsTask = (workItemReason) => workItemReason === 'DEATHARREARS';
const isDeathOverPaymentTask = (workItemReason) => workItemReason === 'DEATHOVERPAYMENT';
const isDeathOverPaymentEmailTask = (workItemReason) => workItemReason === 'DEATHOVERPAYMENTEMAIL';
const isSrbOverPayment = (workItemReason) => workItemReason === 'SRBOVERPAYMENT';
const isAwardOverpayment = (totalAmount) => totalAmount < 0;
...
module.exports = {
  taskDetail(req, workItemReason, award) {
    ...
    if (isDeathArrearsTask(workItemReason)) {
      const details = taskDeathArrearsDetailObject.formatter(award, workItemReason);
      return {
        view: defaultView,
        data: details,
      };
    ...
    if (isDeathOverPaymentTask(workItemReason)) {
      const details = taskDeathOverPaymentDetailObject.formatter(award, workItemReason);
      return {
        view: defaultView,
        data: details,
      };
    ...
    if (isDeathOverPaymentEmailTask(workItemReason)) {
      const details = taskDeathOverPaymentEmailDetailObject.formatter(award, workItemReason);
      return {
        view: defaultView,
        data: details,
      };
    ...
    if (isSrbOverPayment(workItemReason)) {
      const details = taskSrbOverPaymentDetailObject.formatter(award, workItemReason);
      return {
        view: 'overpayment',
        data: details,
      };
  ...
  async taskEnd(req, res, workItemReason) {
    ...
    if (isDeathArrearsTask(workItemReason) || isDeathOverPaymentTask(workItemReason) || isDeathOverPaymentEmailTask(workItemReason)) {
      await updateWorkItem(req, res);
      return ['tasks', 'awardDetails'];
    }
