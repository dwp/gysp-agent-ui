...
function overseasAddress(details) {
  const address = [];

  for (let i = 1; i < 6; i++) {
    const line = details[`line${i}`];
    if (line) {
      address.push(line);
    }
  }

  address.push(details.country);

  return address;
}

module.exports = {
  residentialAddress(details) {
  ...
  address(details) {
    return details.residentialAddress
      ? this.residentialAddress(details.residentialAddress)
      : overseasAddress(details.overseasAddress);
  },
  addressToHtmlLines(details) {
    const address = this.residentialAddress(details);
