...
const maritalDateToStatus = {
  marriageDate: 'married',
  civilPartnershipDate: 'civil',
  divorcedDate: 'divorced',
...
module.exports = {
  ...
  maritalDateSuccessAlert(currentStatus, newShortStatus, verification, suffix = '') {
    ...
    if (this.hasMaritalStatusChanged(currentShortStatus, newShortStatus)) {
      return i18n.t(`marital-status:success-message${suffix}`);
    }
  ...
  previousMaritalDate(partnerDetail) {
    if (!isNotUndefinedEmptyOrNull(partnerDetail) || typeof partnerDetail !== 'object') {
      return false;
    }
    if (Object.keys(partnerDetail).length === 0) {
      return false;
    }
    const allSavedMaritalDates = Object.keys(partnerDetail)
      .filter((key) => key.includes('Date') && partnerDetail[key] !== null)
      .reduce((obj, key) => {
        const pd = obj;
        pd[key] = partnerDetail[key];
        return pd;
      }, {});

    if (Object.keys(allSavedMaritalDates).length === 1) {
      return false;
    }

    // sort by latest date
    const maritalDates = Object.entries(allSavedMaritalDates)
      .map(([key, value]) => ({ status: maritalDateToStatus[key], date: value }))
      .sort((a, b) => b.date - a.date);

    return maritalDates[1];
  },
};
