...
const suffixRegEx = '([A-Da-d]){1}';
const amountRegEx = '^([0-9]){3}\\.([0-9]){2}$';
const protectedPaymentAmountRegEx = '^([0-9]){1,3}\\.([0-9]){2}$';
...
function isSingleCharactor(value) {
  if (value.length <= 1) {
    return true;
  }
  return false;
...
function isValidSuffix(value) {
  if (isSingleCharactor(value)) {
    const suffixCheck = new RegExp(suffixRegEx);
    if (suffixCheck.test(value)) {
      return true;
    }
    return false;
  }
...
function isValidNSP(value) {
  const nspCheck = new RegExp(amountRegEx);
  if (nspCheck.test(value)) {
    return true;
  }
  return false;
}
...
function isValidProtectedPayment(value) {
  const ppCheck = new RegExp(protectedPaymentAmountRegEx);

  if (ppCheck.test(value)) {
    return true;
  }
  return false;
}
...
module.exports = {
  ...
  validateSuffixAndAmounts(form) {
    const errors = [];

    if (form.suffix === undefined || form.suffix === '') {
      errors.suffix = {
        text: i18n.t('enter-amounts:errors.suffix.required'),
      };
    } else if (!isValidSuffix(form.suffix)) {
      errors.suffix = {
        text: i18n.t('enter-amounts:errors.suffix.format'),
      };
    }
    if (form.nsp === undefined || form.nsp === '') {
      errors.nsp = {
        text: i18n.t('enter-amounts:errors.nsp.required'),
      };
    } else if (!isValidNSP(form.nsp)) {
      errors.nsp = {
        text: i18n.t('enter-amounts:errors.nsp.format'),
      };
    }
    if (form.protectedPayment === undefined || form.protectedPayment === '') {
      return errors;
    } if (!isValidProtectedPayment(form.protectedPayment)) {
      errors.protectedPayment = {
        text: i18n.t('enter-amounts:errors.protected_payment.format'),
      };
    }
    return errors;
  },
  ...
  paymentFrequency(form) {
    const errors = [];
    if (form.frequency === '' || form.frequency === undefined) {
      errors.frequency = {
        text: i18n.t('payment-frequency:fields.frequency.errors.required'),
      };
    } else if (!generalHelper.checkIfValidPaymentFrequency(form.frequency)) {
      errors.frequency = {
        text: i18n.t('payment-frequency:fields.frequency.errors.required'),
      };
    }
