  ...
  formatter(detail, id, awardStatus) {
    ...
    object.detailsSummaryRows = [
      ...
      },
      {
        ...
        value: { html: `${dateHelper.longDate(detail.startDate)} to<br />${dateHelper.longDate(detail.endDate)}`, classes: 'payment-detail__period' },
      },
      {
    ...
    if (detail.status === 'PAID' && dateHelper.daysBetweenNowDate(detail.creditDate) <= maxDaysAllowedToChangePaidStatus) {
