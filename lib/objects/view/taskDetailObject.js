...
function partnerDetailFormatter(details, status, updateDetails) {
  const {
  ...
  const { partnerDetail: PD } = details;
  const formatted = {
  ...
  if (status === 'married') {
    formatted.maritalVerified = updatedMaritalVerified || PD.marriageDateVerified;
    if (isNotUndefinedEmptyOrNull(PD.marriageDate)) {
      formatted.maritalDate = longDate(PD.marriageDate);
    }
    if (isNotUndefinedEmptyOrNull(updatedMaritalDate)) {
      formatted.maritalDate = dateDashToLongDate(updatedMaritalDate);
    }
  }
  if (status === 'civil') {
    formatted.maritalVerified = updatedMaritalVerified || PD.civilPartnershipDateVerified;
    if (isNotUndefinedEmptyOrNull(PD.civilPartnershipDate)) {
      formatted.maritalDate = longDate(PD.civilPartnershipDate);
    }
    if (isNotUndefinedEmptyOrNull(updatedMaritalDate)) {
      formatted.maritalDate = dateDashToLongDate(updatedMaritalDate);
    }
...
function partnerSummary(details, updatedEntitlementDetails) {
  const status = maritalStatusHelper.transformToShortStatus(details.maritalStatus);
  const partnerDetail = partnerDetailFormatter(details, status, updatedEntitlementDetails);
  const summary = {
