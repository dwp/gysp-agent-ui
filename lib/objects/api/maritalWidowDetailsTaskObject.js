...
module.exports = {
  formatter(details, awardDetails) {
    const { date } = details;
    const { entitledInheritableStatePension } = details['entitled-to-inherited-state-pension'];
    ...
    const requestBody = {
      ...
      entitledInheritableStatePension: entitledInheritableStatePension === 'yes',
      widowedAmounts: null,
    };

    if (entitledInheritableStatePension === 'yes') {
      requestBody.widowedAmounts = {
