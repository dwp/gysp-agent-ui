...
describe('timeline helper', () => {
  ...
  it('should return formatted contact timeline with list', async () => {
  ...
  it('should return formatted contact timeline with no list', async () => {
    nock('http://test-url/').get(`${auditApiUri}/TEST`).query({ page, limit }).reply(httpStatus.OK, validResponse);
    const response = await helper.getTimeline(validRequest, genericResponse, contactCategory);
    assert.equal(JSON.stringify(response), JSON.stringify(validContactNoListJson));
  });

  it('should return formatted contact timeline with no list when response is 404', async () => {
    nock('http://test-url/').get(`${auditApiUri}/${contactCategory}`).query({ page, limit }).reply(httpStatus.NOT_FOUND, validResponse);
    const response = await helper.getTimeline(validRequest, genericResponse, contactCategory);
    assert.equal(JSON.stringify(response), JSON.stringify(validContactNoListJson));
  });

  it('should return formatted contact timeline with no list when response is 400', async () => {
